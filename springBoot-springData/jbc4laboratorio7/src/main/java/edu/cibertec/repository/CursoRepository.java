...
public interface CursoRepository extends JpaRepository<CursoEntity, Integer> {
    ...
    //**Consulta por estado
    @Query("SELECT c FROM CursoEntity c WHERE c.estado=?1") //En jpql esta es la forma mas utilizada para mapear los parametros
    ...
    @Query(value = "CALL Curso_Por_Nombre(?1)",nativeQuery = true) //Se recomienda invocar los store procedures desde una consulta nativa
